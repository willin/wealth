  ...
  return (
    <g>
      ...
      <text
        ...
        fill={fill}>{`${formatMoney(value as number)}`}</text>
      <text
        x={ex + (cos >= 0 ? 1 : -1) * 12}
        y={ey}
        dy={18}
...
export function PieView({
  ...
  data.forEach((item) => {
    ...
    const index = detail[item.type as InvoiceType].findIndex(
      (x) => x.category === category?.label
    );
    if (index > -1) {
          ...
          <BarChart
            ...
            <Tooltip
              formatter={(value: string, key: string) => [
                `${formatMoney(+value)} (${
                  Math.ceil((+value / summary.IN) * 1e4) / 100
                }%)`,
                t[key]
              ]}
            />
      ...
      <div
        ...
        <ResponsiveContainer>
          <PieChart width={300} height={300}>
            <Legend
              formatter={(_, { payload }) =>
                (payload as any as Invoice).category
              }
            />
            <Pie
