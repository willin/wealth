  ...
  if (month % 3 === 1) {
    return (
      <text
        ...
        y={y - 4}
        textAnchor='middle'
        fill='currentcolor'>{`Q${quarterNo}`}</text>
    );
  }
        ...
        <div style={{ width: '100%', height: 300 }}>
          <ResponsiveContainer>
            <ComposedChart width={500} height={300} data={grids}>
              ...
              <YAxis
                hide={width < 768}
                tickFormatter={(v: any) => `${Math.ceil(v / 100) / 10}k`}
              />
              <Tooltip
              ...
              <Bar dataKey='IN' barSize={20} fill='oklch(var(--p))' />
              <Bar dataKey='OUT' barSize={20} fill='oklch(var(--s))' />
              <Line
                type='monotone'
                dataKey='BALANCE'
                stroke='oklch(var(--a))'
